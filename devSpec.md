...
- 存储方式：通过Pinia Store管理，按业务域垂直拆分（如`useCardStore`、`useSyncStore`）
- 示例：各模式的授权配置、卡片基础结构的全局索引

#### 5.2.2 组合式函数（Composables）设计规范
1. **命名规范**：
   - 以`use`为前缀，后跟描述性名称（如`useCardEditor`、`useSyncHandler`）
...
3. **Action设计**：
   ...
   - action专注于状态更新，复杂业务逻辑抽离到独立的业务逻辑模块
...
| 层级 | 技术实现 | 核心职责 | 禁止行为 |
|------|---------|---------|---------|
| UI层 | 1. 核心：`UniversalCard.vue`（受控组件，整体使用）<br>2. 辅助：其他业务组件（.vue） | 1. `UniversalCard`：负责核心卡片渲染、内部字段交互、接收外部命令<br>2. 其他组件：辅助展示与业务场景适配<br>3. 所有UI组件通过组合式函数与状态层交互 | 1. 禁止修改`UniversalCard`内部实现<br>2. 禁止绕过组合式函数直接操作状态管理层<br>3. 禁止在UI组件中包含复杂业务逻辑 |
| 状态管理层 | 组合式函数（composables/*.js）、Pinia Stores（stores/*.js） | 1. 组合式函数：管理局部状态（ref/reactive），封装组件内业务逻辑（如表单验证、临时数据处理），作为`UniversalCard`与系统其他模块的"通信桥梁"<br>2. Pinia：管理全局共享状态，提供跨组件数据访问和修改接口（遵循State→Getters→Actions单向流）<br>3. 统一处理null→空白等数据转换 | 1. 组合式函数禁止直接修改Pinia State（需通过Actions）<br>2. Pinia禁止存储局部状态（如仅某组件使用的UI状态）<br>3. 两者均不直接操作DOM |
...
2. **数据加载流程**：
   ```
   组合式函数 → Pinia Action → API模块 → 后端服务/本地存储
                                          ↓
   组合式函数 ← Pinia State ← API模块 ← 后端服务/本地存储
   ```
...
1. **命令封装**：
   对`UniversalCard`的所有操作（如更新字段值、切换编辑状态、同步数据）必须通过专用组合式函数封装，禁止在组件外直接调用其内部方法。
   ...
   export function useUniversalCardController(cardId) {
     ...
     function updateField(fieldKey, value) {
       // 1. 先通过状态层校验（如授权判断）
       const syncStatus = getSyncStatus(cardId, fieldKey)
     ...
     function syncFromGlobal() {
       const globalData = useCardStore().getCardData(cardId)
...
2. **状态同步**：
   `UniversalCard`的内部状态变化（如用户输入、交互反馈）需通过`emit`触发事件，由组合式函数接收并同步至状态管理层，禁止组件直接修改Pinia状态。
   示例：组件内触发事件→组合式函数接收→同步至Store：

   ```javascript
   // 在组件中使用
   const { cardRef, updateField } = useUniversalCardController(cardId)
   
   // 监听组件内部变化，同步至全局状态
   function handleCardChange(fieldKey, value) {
     useCardStore().updateCardField(cardId, fieldKey, value)
   }
   ```
...
- 设定syncStatus（同步/授权规则，引用5.2节），通过Pinia的useSyncStore().setSyncStatus(config)持久化全局同步配置
...
1. **组件控制封装**：
   ...
   - 组合式函数应封装所有与组件的交互细节，对外提供简洁接口

2. **状态同步模式**：
   ```javascript
   // 标准同步模式示例
   function useSyncedCard(cardId) {
     // 1. 获取全局状态
     const cardStore = useCardStore();
     const cardData = computed(() => cardStore.getCard(cardId));
     ...
     const handleTitleChange = (newTitle) => {
       cardStore.updateCardField(cardId, 'title', newTitle);
     };
     
     // 4. 提供操作方法
     const methods = {
       enterEditMode() {
         cardStore.setCardMode(cardId, 'edit');
       },
       saveCard() {
         return cardStore.saveCard(cardId);
       }
     };
...
2. **命名规范**：必须以`use`为前缀，如`useCardEditor`、`useDataValidator`
3. **参数明确**：输入输出参数清晰，避免隐式依赖
4. **返回值一致**：返回对象中，状态使用ref/computed，方法使用普通函数
5. **可组合性**：设计时考虑与其他组合式函数的配合使用
...
2. **状态转换**：组合式函数负责将Store中的原始数据转换为UI友好的格式
3. **避免冗余**：不重复存储Store中已有的状态
4. **示例**：
   ```javascript
   function useFormattedCard(cardId) {
     const cardStore = useCardStore();
     const card = computed(() => cardStore.getCard(cardId));
     ...
     return {
       formattedOptions,
       updateOption: (id, data) => cardStore.updateOption(cardId, id, data)
     };
   }
   ```
...
| 组合式函数（Composables） | 封装组件内业务逻辑和局部状态的函数（存放于composables目录），命名以`use`为前缀（如`useCardEditor`），用于实现逻辑复用和代码组织，是本系统推荐的核心编码方式 |
...
3. 数据存储使用浏览器本地存储方案（localStorage/IndexedDB）
